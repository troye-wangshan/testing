...
#if 0
    // Sample code for automatically starting the continuous packet TX test
    {
...
void hci_xtal_trim_cmd(uint8_t* ptr_data)
{
  uint16_t delta = ptr_data[2]<<8|ptr_data[1];
	...
	set_pad_functions(); // xtal trim clk input could be the UART RX. So restore GPIO funcionality here.
...
void hci_custom_action_cmd(uint8_t *ptr_data)
{
	...
	bufptr = (uint8_t*)&s;
	uart_write(bufptr, sizeof(struct msg_custom_action), NULL);
}
...
void hci_fw_version_cmd(void)
{
	...
	memcpy(&s.ble_ver, DA14580_SW_VERSION, 32);
	s.app_ver_len  = sizeof(DA14580_REFDES_SW_VERSION);
	memcpy(&s.app_ver, DA14580_REFDES_SW_VERSION, 32);
	bufptr = (uint8_t*)&s;
	uart_write(bufptr, sizeof(struct msg_fw_version_action), NULL);
}
